...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        return scheme_apply(scheme_eval(first, env), rest.map(lambda arg:scheme_eval(arg, env)), env)
        # END PROBLEM 4
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        self_frame = self
        while True:
            if symbol in self_frame.bindings:
                return self_frame.bindings[symbol]
            else:
                self_frame = self_frame.parent
                if self_frame is None:
                    break
                return self_frame.lookup(symbol)
        # END PROBLEM 2
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        # print(expressions.rest.first)
        # print(do_lambda_form(Pair(target.rest, Pair(expressions.rest.first, nil)), env))
        # print(target.first)
...
def make_let_frame(bindings, env):
    ...
    let_formals = []
    while bindings != nil:
        let_formals.append(bindings.first.first)
        if bindings.first.rest == nil or\
            bindings.first.rest.rest != nil or\
            let_formals.count(bindings.first.first) > 1 or\
            type(bindings.first.first) != str:
            raise SchemeError
