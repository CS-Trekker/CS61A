...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_a_distance = distance(make_city("temp", lat, lon), city_a)
    city_b_distance = distance(make_city("temp", lat, lon), city_b)
    return get_name(city_a) if city_a_distance < city_b_distance else get_name(city_b)
...
def berry_finder(t):
    ...
    if is_leaf(t):
        ...
        if label(t) == "berry":
            ...
            return any(berry_finder(b) for b in branches(t))   # 这里的any函数用得好
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[0] if (1 - len(deck[i:]) % 2) else deck[i + 1:][(len(deck) - i - 2) // 2] for i in range(len(deck))]
